...
export default async (api, message) => {
  ...
  if (action.type === 'join') {
    ...
    return api.emit(subscriber.id === api.currentSubscriber.id ? Event.JOINED_GROUP : Event.GROUP_MEMBER_ADD, group, subscriber);
  }
  let existing = group._members.find((subscriber) => subscriber.subscriberId === subscriber.id);
  if (!existing) {
    existing = {
      groupId: group.id,
      subscriberId: subscriber.id
  ...
  if (action.type === 'leave' || action.type === 'kick') {
    ...
    return api.emit(subscriber.id === api.currentSubscriber.id ? Event.LEFT_GROUP : Event.GROUP_MEMBER_DELETE, group, subscriber);
  }
