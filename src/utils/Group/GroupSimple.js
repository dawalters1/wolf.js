...
class GroupSimple {
  ...
  async getAvatar (targetGroupId, size) {
    try {
      ...
      return await this._api._utility._download.file(
        this._api._utility._string.replace(`${this._api.endpointConfig.avatarEndpoint}/FileServerSpring/group/avatar/{targetGroupId}?size={size}`,
          {
            targetGroupId,
            size
          }
