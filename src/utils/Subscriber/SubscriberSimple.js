...
class SubscriberSimple {
  ...
  async getAvatar (subscriberId, size) {
    try {
      ...
      return await this._api._utility._download.file(this._api._utility._string.replace(`${this._api.endpointConfig.avatarEndpoint}/FileServerSpring/subscriber/avatar/{subscriberId}?size={size}`,
        {
          subscriberId,
          size
        }
  ...
  async hasCharm (subscriberId, charmIds, requireAll = false) {
    try {
      ...
      const summary = subscriber.charmIds || ((await this._api._charm.getSubscriberSummary(subscriberId)).body || []).map((charm) => charm.charmId);
