...
const removePunctuation = (url) => {
  if (url && url.slice(-1).match(PUNCTUATION)) {
    url = removePunctuation(url.slice(0, url.length - 1));
  }

  if (url && url.slice(0).match(PUNCTUATION)) {
    url = removePunctuation(url.slice(1));
  }
...
class String {
   ...
   * @param {Object{key: string, value: Number|string}} replacements - What to replace and with what
   * @returns
   */
  ...
  /**
   * See if two strings are the same (Not case sensitive)
   * @param {String} sideA - The first string to check
   * @param {String} sideB - The second string to check
   * @returns {Boolean}
   */
  isEqual (sideA, sideB) {
  ...
  /**
   * Chunk a string into specific lengths
   * @param {String} string - The string to chunk
   * @param {Number} max - How long each chunk should be
   * @param {String} splitChar - The character used to split
   * @param {String} joinChar - The character used to rejoin lines
  ...
  /**
   * Remove ads from strings
   * @param {String} string - The string to remove ads from
   * @returns {String} - String with ads removed from it
   */
  trimAds (string) {
    if (typeof (string) !== 'string') {
  ...
  isValidUrl (url) {
    ...
    if (url && (url.includes('.') || url.includes(':'))) {
      const protocol = PROTOCOLS.sort((a, b) => b.length - a.length).find((proto) => url.toLowerCase().startsWith(proto)) || 'http://';
      ...
      if (url.split('.').length > 2) {
        try {
          ...
          if ((data.hostname.includes('.') && TLD.includes(removePunctuation(data.hostname.split('.').pop()))) || (data.host.includes(':') && data.port)) {
            return {
              original,
              formatted: data.href
            };
