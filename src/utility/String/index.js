...
class StringUtility extends Base {
  ...
  getValidUrl (string) {
    ...
    } catch (error) { // if something is malformed with provided data to URL it will throw an error
      return undefined;
    }
  }
  ...
  getAds (string) {
    ...
    return ([...string.matchAll(/(?:(?<!\d|\p{Letter}))(\[(.+?)\])(?:(?!\d|\p{Letter}))/gu)] || []).map((result) => ({
      start: result.index,
