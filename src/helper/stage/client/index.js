...
const { events, state } = require('./constants');
...
class WRTCWrapper {
    ...
    this._client.onconnectionstatechange = async () => {
      if (this._client.connectionState === 'connected') {
        if (this._connectionState === state.CONNECTING) {
          ...
          return this._em.emit(events.READY, {});
        }
        ...
        return this._em.emit(events.DISCONNECTED, this._getData());
      }
  ...
  async _broadcast () {
    ...
    if (this._duration % 1000 === 0) {
      this._em.emit(events.DURATION, this._getData());
    }
  ...
  handleSlotUpdate (slot, sourceSubscriberId) {
    if (slot.occupierId !== null) {
      if (this._muted && !slot.occupierMuted) {
        this._muted = false;
        return this._em.emit(events.UNMUTED, this._getData(sourceSubscriberId));
      } else if (!this._muted && slot.occupierMuted) {
        this._muted = true;
        return this._em.emit(events.MUTED, this._getData(sourceSubscriberId));
      } else if (!slot.locked) {
        return Promise.resolve();
      }
    }
    ...
    this._em.emit(sourceSubscriberId !== undefined ? events.KICKED : events.DISCONNECTED, this._getData(sourceSubscriberId));
  }
  ...
  broadcast (data) {
    ...
    this._ffmpeg = ffmpeg(this._downloader)
      ...
      .on('error', (error) => {
        ...
        this._em.emit(events.ERROR, { error });
      })
      .pipe()
      .on('data', (chunk) => {
  ...
  stop () {
    ...
    this._em.emit(events.STOP, this._getData());
  ...
  pause () {
    ...
    this._em.emit(events.PAUSED, this._getData());
  ...
  resume () {
    ...
    this._em.emit(events.RESUME, this._getData());
  ...
  onEnd (fn) { this._em.on(events.END, fn); }
  onDuration (fn) { this._em.on(events.DURATION, fn); }
  onStop (fn) { this._em.on(events.STOP, fn); }
  onError (fn) { this._em.on(events.ERROR, fn); }
  onConnecting (fn) { this._em.on(events.CONNECTING, fn); };
  onConnected (fn) { this._em.on(events.CONNECTED, fn); }
  onKicked (fn) { this._em.on(events.KICKED, fn); }
  onMuted (fn) { this._em.on(events.MUTED, fn); }
  onUnmuted (fn) { this._em.on(events.UNMUTED, fn); }
  onDisconnected (fn) { this._em.on(events.DISCONNECTED, fn); };
  onReady (fn) { this._em.on(events.READY, fn); }
  onPaused (fn) { this._em.on(events.PAUSED, fn); }
  onResume (fn) { this._em.on(events.RESUME, fn); }
