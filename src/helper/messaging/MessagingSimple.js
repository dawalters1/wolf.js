...
class MessagingSimple extends BaseHelper {
  ...
  async _sendMessage (targetType, targetId, content, opts = {}) {
    ...
    if (this._api._botConfig.get('multimedia.messaging.validation.mimes').includes(mimeType)) {
      return await this._api._multiMediaService.sendMessage(targetType, targetId, content, mimeType);
    }
    ...
    if (!_opts.chunk && content.length > messageLength.max) {
      throw new Error(`Maximum allowed message length with chunking disabled is 1,000, provided message is ${this._api._utility._number.addCommas(content.length)}`);
    }
    ...
    const chunkedMessage = this._api._utility._string.chunk(content.split(' ').filter(Boolean).join(' '), (_opts.chunk ? _opts.chunkSize : content.length) - (!_opts.formatting.alert && !_opts.formatting.me ? 0 : _opts.formatting.alert ? 6 : 3), ' ', ' ');
    ...
    const messagesToSend = (await chunkedMessage.reduce(async (result, chunk, index) => {
      ...
      const adsInChunk = this._api._utility._string.getAds(chunk);
      ...
      const linksInChunk = chunk.split(' ').reduce((result, value, index) => {
        if (validator.isValidUrl(this._api, value)) {
          ...
          if (!adsInChunk.some((ad) => inRange(ad.index, ad.index + ad[0].length, linkStartLocation) && inRange(ad.index, ad.index + ad[0].length, linkEndLocation))) {
            const url = this._api._utility._string.getValidUrl(value);
          ...
          linksInChunk.push(
            {
              ...
              url: link.type !== MessageLinkingType.EXTERNAL
                ? this._api._utility._string.replace(this._api._botConfig.get('deeplinks')[link.type],
                  {
                    value: link.value
                  }
                )
      ...
      if (linksInChunk.length > 0 || adsInChunk.length > 0) {
        ...
        if (adsInChunk.length > 0) {
          body.metadata.formatting.groupLinks = await adsInChunk.reduce(async (result, value) => {
            ...
            const group = await this._api._group.getByName(value[1]);
            (await result).push(
              {
                start: value.index,
